...
/*
  The US now requires applications generating text messages using 10 digits long code numbers to define message use cases
  Each use case has to be assigned to a set of unique phone numbers.
  Failure to comply will result in lower successful delivery rate for these text messages.
  The defined use cases can be found in our Tetrascience Twilio account. Using these sid's as from for text messages
  will map to phone numbers assigned to these messaging services automatically.

 */
const twilioMessagingServices = {
  notifications: 'MG2396ea3e9c077aee27063d69bf4b1841',
  misc: 'MG44b76e35ebc3150c3ab6caa60f6f34fc'
};
...
function initMailer(config) {
  if (config.provider_email === 'smtp') {
    ...
    if (config.smtp_limit_rate) {
      ...
      smtpConf.maxMessages = config.smtp_max_messages;
      smtpConf.pool = true;
      smtpConf.maxConnections = 1;
      smtpConf.rateDelta = config.smtp_rate_delta;
    }
    ...
    return nodemailer.createTransport(new AWS.SES({
...
module.exports.sendText = function (notification) {
  let fromMessagingService;

  switch(notification.type) {
    case 'sensor-trigger':
    case 'sensor-trigger-state':
    case 'online-alert':
    case 'offline-alert':
      fromMessagingService = twilioMessagingServices.notifications;
      break;
    default:
      fromMessagingService = twilioMessagingServices.misc;
      break;
  }
